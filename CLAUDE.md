...
- `FoodAddSheet` (the serving/quantity picker) renders as a `position:fixed` bottom sheet at `zIndex:1100`. The "ADD TO [MEAL] ◆" confirm button is in a sticky footer with 100px bottom padding to clear the nav bar — use `{ force: true }` in Playwright. Close via ✕ button or backdrop click — there is no "CANCEL" button. Fraction chip labels are ASCII (`1/2`, `1/3`, etc.) — test selectors must use these strings, not Unicode glyphs like `½`.
...
**Critical:** The Supabase secret name must be exactly `ANTHROPIC_API_KEY` — all caps, underscore-separated. The functions call `Deno.env.get("ANTHROPIC_API_KEY")` and any spelling or case variation (e.g. `Anthropic_API_key`) causes silent failure where the meal scanner and form check return errors despite the functions appearing deployed.
...
Render guards (in order): animated splash screen while `authReady` is false → `<ResetPasswordScreen/>` when `passwordRecovery` is true → `<LoginScreen/>` when no user → "CONNECTION ERROR" screen with Retry button when `!profile && dataLoadFailed` (network errors in `loadProfile`/`loadSessions` set this flag; a missing profile row — Postgres error `PGRST116` — does not) → animated splash while profile loads (i.e. `user` set but `profile` still null) → main app with overlay stack: `OnboardingModal` (zIndex 850) → `ProfileSetupModal` (zIndex 820) → `DailyMotivModal` (zIndex 800) → `GuidedTourOverlay` (triggered after ProfileSetupModal completes/skips). `DailyMotivModal` is suppressed while onboarding or profile setup is active. `GuidedTourOverlay` fires once per device (guarded by `sfc_tour_done`), triggered with a 400ms delay after ProfileSetupModal's onDone callback.

`dataLoadFailed` is reset to `false` at the start of every `onAuthStateChange` event before calling `ensureProfile`. This ensures that if `loadProfile` fails on `INITIAL_SESSION` (expired token, network not ready on iOS wake), the subsequent `TOKEN_REFRESHED` event clears the error and retries cleanly without leaving the CONNECTION ERROR screen stuck.
...
**Programs** — `PROGRAMS_DATA` is a module-level constant with **17 complete workout programs** (each has `id`, `name`, `type`, `goal`, `days`, `weeks`, `daysPerWeek`, `level`, `ico`, `col`, `equipment`, `schedule`, and `days: [{ name, tag, exs: [{ name, sets, reps }] }]`). Program types: `BODYBUILDING`, `POWERLIFTING`, `HYPERTROPHY`, `CALISTHENICS`, `FUNCTIONAL`, `ATHLETICS`, `SPORTS`, `CORRECTIVE`, `WEIGHT LOSS`. Tapping a program card opens `ProgramDetailModal` (full-screen, not a bottom sheet): shows goal, a 3-stat grid (days/duration/level), a weekly schedule strip with day selector, an exercise list for the selected day, and a "LOG DAY X WORKOUT" button. That button pre-loads exercise names into `exs`, sets `sessName`, and switches the `subTab` to `"track"` via `onStartDay` prop. `TrainScreen` owns `selectedProgram` and `progFilter` state. A horizontally scrollable category filter chip row (ALL / BODYBUILDING / POWERLIFTING / … / CORRECTIVE) sits above the program list and filters `PROGRAMS_DATA` by `p.type`. Adding a new program: append an entry to `PROGRAMS_DATA` and make sure `type` matches one of the filter chip labels (or add the label to the chip row).
...
**Bodyweight auto-fill** — `BODYWEIGHT_EXERCISES` is a module-level `Set` of exercises that should default to the user's logged body weight. Currently includes Pull-Ups, Chin-Ups, Push-Ups, Dips, Chest Dip, and all bodyweight core exercises (Plank, Side Plank, Ab Wheel, Hanging Leg Raises, Leg Raises, Russian Twists, Bicycle Crunches, Crunch, Decline Crunches, Sit-Ups, V-Ups, Dragon Flag, Dead Bug, Flutter Kicks, Mountain Climbers, Hollow Body Hold, Toes-to-Bar, Windshield Wipers). When selected, `selectExercise` pre-fills the weight field with `getLoggedBodyWeight()` and a teal "BODYWEIGHT EXERCISE" strip appears above the set rows with a USE BW button. The column header changes to `"WEIGHT (LBS) ⚖️"`. Adding an exercise to this set: append its name to `BODYWEIGHT_EXERCISES` and also add a MET value in `EXERCISE_MET`.

**Calories burned** — a live `KCAL 🔥` tile appears in the gold stats strip (SETS / VOLUME / PTS / KCAL 🔥) as soon as any working set is logged, provided `getLoggedBodyWeight()` returns a value. Also shown in the save card subtitle (`· ~X kcal`) and on every HISTORY card (`🔥 ~X kcal`). The tile is hidden entirely when no body weight is on record. Calories reflect active set time + inter-set rest only — not the full session clock — so numbers are lower than full-session trackers like Apple Watch.
...
4. **MY BADGES card** — tappable showing unlock count ("X of 29 unlocked") and up to 9 unlocked badge emoji tiles (or 5 locked placeholders for new users). Tapping opens `AchievementsModal`. State: `badgesOpen` + `unlockedBadges` (computed from `getUnlockedBadges(sessions, profile, extraData)` each render — `extraData` is read from localStorage the same way `AchievementsModal` does, so BODY and NUTRITION badges reflect actual data).
...
The feed is fully **Supabase-backed** — posts, likes, and comments are all stored in database tables, not localStorage.
...
**`FoodAddSheet`** — a `position:fixed` bottom sheet (`zIndex:1100`) opened by `openFoodAdd(food)`. State: `addingFood` (the food object or null), `foodQty` (fraction: 0.25–3), `foodCount` (integer: 1–10+), `foodUnit` (`"serving"` | `"cup"`). Layout: fixed header (food name + ✕ close), scrollable body (QUANTITY chips 1–10 + −/+ stepper, unit toggle 📋 SERVING | 🥄 CUP, fraction chips using ASCII text `1/4 1/3 1/2 2/3 3/4 1 1 1/4 1 1/2 2 3` + custom number input — **do not use Unicode vulgar fraction glyphs** as Bebas Neue does not render them correctly), live macro preview (CAL/P/C/F), and a sticky gold ADD TO [MEAL] ◆ footer with `paddingBottom:"calc(env(safe-area-inset-bottom,0px) + 100px)"` to clear the nav bar. Body scroll-lock is applied via a `useEffect` on `addingFood` (not `useScrollLock()` — the sheet is rendered inline inside an IIFE, not a component). Background scroll-steal is blocked via `onTouchMove={e=>e.preventDefault()}` on the backdrop and `onTouchMove={e=>e.stopPropagation()}` on the panel and scrollable body. Close via ✕ header button or backdrop click. `confirmFoodAdd` scales macros by `frac × count`, stores `servingLabel` on the logged item, appends to `log`, and sets `addingFood(null)`.
...
`EXERCISE_TIPS` maps 232 exercise names → `{ cues: [string, string, string], err: string }`. Used by `ExerciseDemoModal`. Keys must exactly match the strings in `EXERCISE_CATS` (case-sensitive) — mismatched keys silently show "No form guide available." Adding a new exercise tip: add an entry here; no other constant needs updating.
...
- **`isSavingRef` guards the visibilitychange handler**: `handleSave` sets `isSavingRef.current = true` before the Supabase insert and clears it after `loadSessions` resolves (on both success and error paths). The `visibilitychange` handler skips `loadSessions` while `isSavingRef.current` is true — this prevents an iOS background/foreground cycle during the async insert from overwriting the optimistic state with the pre-save DB snapshot, which was causing sessions to not appear after saving on first try.
